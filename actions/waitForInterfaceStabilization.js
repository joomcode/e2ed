...
const clientWaitForInterfaceStabilization = (0, testcafe_1.ClientFunction)((stabilizationInterval) => {
    const global = window;
...
const waitForInterfaceStabilization = async (stabilizationInterval = 500) => {
    const waitInMs = await clientWaitForInterfaceStabilization(stabilizationInterval);
    (0, utils_1.log)(`Waited for interface stabilization for ${waitInMs} ms with stabilization interval ${stabilizationInterval}`);
};
